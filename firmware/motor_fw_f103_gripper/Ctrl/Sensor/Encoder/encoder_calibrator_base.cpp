...
void EncoderCalibratorBase::TickMainLoop()
{
    ...
    if (errorCode == CALI_NO_ERROR)
    {
        ...
        // P0-10: verify Flash write completed by checking address reached expected position.
        extern bool Stockpile_Flash_Data_WriteComplete(Stockpile_FLASH_Typedef*, uint32_t);
        extern Stockpile_FLASH_Typedef stockpile_quick_cali;
        if (!Stockpile_Flash_Data_WriteComplete(&stockpile_quick_cali, resultNum))
            errorCode = CALI_ERROR_VERIFY_FAIL;
